...
## Do downscaling (coarser resolutions)
...
base_data.aggs.df = base_data.aggs |>
  lapply(FUN=function(d) {
    d = as.data.frame(d)
    d[apply(d,1,FUN=function(x) !any(is.na(x))),]
  })
```
...
```{r prediction_helper}
#Notice that this part (make_quadratic, predict.make_quadratic and maxent_param below) comes from Valavi et al 2022 in Ecological Monograph
...
run_model_lasso = function(df,lines,predictors) {
  ...
  return(list(quad_obj=quad_obj,lassomod=lassomod))
}
...
run_model_maxent = function(df,lines,predictors) {
  ...
  fp = tempfile("results/maxent.outputs.",tmpdir=".")
...
```{r predictfuns}
predict_model_gam = function(model,df,lines,bd,predictors) {
  library(stars)
  bdf = as.data.frame(bd)
  pp = predict(model,bdf,type="response")
  
  bd$predgam = as.numeric(pp)

  return(bd["predgam"])
}

predict_model_lasso = function(model,df,lines,bd,predictors) {
  library(stars)
  bdf = as.data.frame(bd)[,predictors]
  bdf_quad <- predict.make_quadratic(model$quad_obj, newdata = bdf)
  bdf_sparse <- Matrix::sparse.model.matrix( ~. -1, bdf_quad)

  pp = predict(model$lassomod,newx=bdf_sparse,type="response",s="lambda.min")
  # predict.glmnet excludes NA's...

  I = apply(bdf,1,function(x)!any(is.na(x)))
  bd$predlasso = NA
  bd$predlasso[I] = as.numeric(pp)

  return(bd["predlasso"])
}

predict_model_maxent = function(model,df,lines,bd,predictors) {
  library(stars)
  bdf = as.data.frame(bd)[,predictors]

  pp = predict(model,bdf)
  # predict.MaxEnt excludes NA's...

  I = apply(bdf,1,function(x)!any(is.na(x)))
  bd$predmaxent = NA
  bd$predmaxent[I] = as.numeric(pp)

  return(bd["predmaxent"])
}

predict_model_rf = function(model,df,lines,bd,predictors) {
  library(stars)
  bdf = as.data.frame(bd)[,predictors]

  pp = predict(model,bdf,type="prob") # DMK: Predicts for all data, but has 2 columns for prob =0, prob =1
  bd$predrf = pp[,"1"]

  return(bd["predrf"])
}

predict_model_gbm = function(model,df,lines,bd,predictors) {
  library(stars)
  predgbm = predict(bd[predictors],model,n.trees=model$gbm.call$best.trees,type="response")
...
res = foreach(n=1:nrow(new_mods),.export=c("params",lsf.str())) %dopar% {
  ...
  preds = do.call(pred_fn,list(model=model,df=df,lines=lines,bd=bd,predictors=predictors))
...
model_stats = foreach(n=nn) %do% {
  ...
  list(scale=sc,split=sp,auc=aa,threshold=th,accuracy=acc,c.stat.boyce=c.stat)
}
